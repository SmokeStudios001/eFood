...
class SearchProvider extends DataSyncProvider {
  ...
  bool _vegTagStatus = false;
  bool _nonVegTagStatus = false;
  
  // Temporary state for filter widget (staged changes before Apply)
  bool _tempVegTagStatus = false;
  bool _tempNonVegTagStatus = false;
  ...
  bool get vegTagStatus => _vegTagStatus;
  bool get nonVegTagStatus => _nonVegTagStatus;
  bool get tempVegTagStatus => _tempVegTagStatus;
  bool get tempNonVegTagStatus => _tempNonVegTagStatus;
  ...
  Future<void> searchProduct({
    ...
    ApiResponseModel apiResponse = await searchRepo!.getSearchProductList(
      ...
      productType: _getProductType(vegTagStatus, nonVegTagStatus),
    );
  ...
  void resetFilterData({bool isUpdate = true}) {
    ...
    _vegTagStatus = false;
    _nonVegTagStatus = false;
    _tempVegTagStatus = false;
    _tempNonVegTagStatus = false;
  ...
  // Update temporary veg/non-veg state (for filter widget staging)
  void onChangeTempVegTagStatus({bool? status}) {
    _tempVegTagStatus = status ?? false;
    notifyListeners();
  }

  void onChangeTempNonVegTagStatus({bool? status}) {
    _tempNonVegTagStatus = status ?? false;
    notifyListeners();
  }

  // Initialize temp state from actual state (when filter opens)
  void initTempFoodTypeStatus() {
    _tempVegTagStatus = _vegTagStatus;
    _tempNonVegTagStatus = _nonVegTagStatus;
    notifyListeners();
  }

  // Commit temporary state to actual state (when Apply is clicked)
  void commitTempFoodTypeStatus() {
    _vegTagStatus = _tempVegTagStatus;
    _nonVegTagStatus = _tempNonVegTagStatus;
    notifyListeners();
  }
  ...
  void onChangeVegTagStatus({bool? status }) {
    _vegTagStatus = status ?? false;
    notifyListeners();
  }

  void onChangeNonVegTagStatus({bool? status }) {
    _nonVegTagStatus = status ?? false;
    notifyListeners();
  }

  /// Helper method to determine product type based on veg/non-veg filter status
  /// Returns 'veg', 'non_veg', or 'all' based on filter combination
  String _getProductType(bool isVeg, bool isNonVeg) {
    // Both selected or both not selected → 'all'
    if (isVeg == isNonVeg) {
      return 'all';
    }
    // Only veg selected → 'veg'
    if (isVeg) {
      return 'veg';
    }
    // Only non-veg selected → 'non_veg'
    return 'non_veg';
  }
