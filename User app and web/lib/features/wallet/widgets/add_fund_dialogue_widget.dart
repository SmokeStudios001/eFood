            ...
            child: Consumer<WalletProvider>(builder: (context, walletProvider, _) {
              return Consumer<CheckoutProvider>(builder: (context, checkoutProvider, _) {
                return Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    if(inputAmountController.text.isNotEmpty) Container(
                      constraints: BoxConstraints(
                        maxHeight: 300,
                        // minHeight: 50,
                      ),
                  ...
                  CustomButtonWidget(
                    ...
                    isLoading: walletProvider.isSubmitLoading,
                    onTap: () async {
                      ...
                      if(inputAmountController.text.isEmpty){
                        ...
                        final paymentUrl = await walletProvider.getAddFundWalletUrl(
                          context,
                          customerId: profileProvider.userInfoModel?.id,
                          paymentMethod: checkoutProvider.paymentMethod?.getWay,
                          paymentPlatform: kIsWeb ? 'web' : 'app',
                          amount: amount,
                          callback: kIsWeb
                              ? '$protocol//$hostname${kDebugMode ? ':$port' : ''}${RouterHelper.wallet}'
                              : '${AppConstants.baseUrl}${RouterHelper.wallet}',
                        );
                        ...
                        if(paymentUrl != null){
                          if(context.mounted){
                            ...
                            if(kIsWeb){
                              ...
                              RouterHelper.getPaymentRoute(paymentUrl, fromCheckout: false);
                            }
                          }
                        }
