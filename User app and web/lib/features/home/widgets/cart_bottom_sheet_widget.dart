...
class _CartBottomSheetWidgetState extends State<CartBottomSheetWidget> {
  ...
  void initState() {
    ...
    // Retrieve existing cart model if available
    final CartModel? existingCart = _getExistingCart();
    
    // Initialize product provider with cart data
    _initializeProductProvider(existingCart);
    
    // Schedule post-frame callback to check cart state after build
    _schedulePostFrameCallback(existingCart);
  }
  
  /// Retrieves the existing cart model from CartProvider if cartIndex is valid
  CartModel? _getExistingCart() {
    // Return widget cart if provided
    if (widget.cart != null) {
      return widget.cart;
    }
    
    // Retrieve cart from provider using cartIndex
    if (widget.cartIndex != null && widget.cartIndex! >= 0) {
      final cartProvider = Provider.of<CartProvider>(context, listen: false);
      
      if (widget.cartIndex! < cartProvider.cartList.length) {
        return cartProvider.cartList[widget.cartIndex!];
      }
    }
  ...
  /// Initializes the ProductProvider with product and cart data
  void _initializeProductProvider(CartModel? cart) {
    final productProvider = Provider.of<ProductProvider>(context, listen: false);
    final product = widget.product;
    
    // Initialize product data with cart information
    productProvider.initData(product, cart);
    
    // Reset product review state
    productProvider.setProductReview = null;
    productProvider.updateProductReviewShowStatus(value: false, isUpdate: false);
    
    // Initialize variation status if variations exist
    final variationsCount = product?.variations?.length ?? 0;
    if (variationsCount > 0) {
      productProvider.initProductVariationStatus(variationsCount);
    }
  }
  
  /// Schedules a post-frame callback to update cart existence state
  void _schedulePostFrameCallback(CartModel? cart) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final product = widget.product;
      if (product == null) return;
      
      final productProvider = Provider.of<ProductProvider>(context, listen: false);
      final variations = cart?.variations ?? [];
      
      // Update cart existence status for the product
      productProvider.setExistInCartForBottomSheet(product, variations);
    });
  }
  
  /// Handles variation selection changes by updating cart state and validating requirements
  /// 
  /// This method consolidates the common logic for:
  /// - Setting the selected variation in the provider
  /// - Updating cart existence status (only when not editing an existing cart)
  /// - Validating required variation selections
  void _handleVariationChange({
    ...
    // Update cart existence status only when adding new items (not editing)
    if (widget.cart == null) {
      productProvider.setExistInCartForBottomSheet(
        widget.product!,
        productProvider.selectedVariations,
      );
    }
    
    // Validate required selections for this variation
    productProvider.checkIsRequiredSelected(
      index: variationIndex,
      isMultiSelect: isMultiSelect,
      variations: productProvider.selectedVariations[variationIndex],
      min: variationList[variationIndex].min,
      max: variationList[variationIndex].max,
    );
  }
    ...
    return Consumer<ProductProvider>(builder: (ctx, productProvider,_){
      return Consumer<CartProvider>(
        builder: (context, cartProvider, child) {
          ...
          final productBranchWithPrice = ProductHelper.getBranchProductVariationWithPrice(widget.product);
          final List<Variation>? variationList = productBranchWithPrice.variatins;
          final double? basePrice = productBranchWithPrice.price;

          // Calculate variation price
          final double variationPrice = ProductHelper.calculateVariationPrice(
            variationList: variationList,
            selectedVariations: productProvider.selectedVariations,
          );

          // Calculate addons data
          final addonsData = ProductHelper.calculateAddonsData(
            product: widget.product,
            addOnActiveList: productProvider.addOnActiveList,
            addOnQtyList: productProvider.addOnQtyList,
          );

          // Calculate all price details
          final priceDetails = ProductHelper.calculatePriceDetails(
            basePrice: basePrice!,
            variationPrice: variationPrice,
            discount: widget.product!.discount,
            discountType: widget.product!.discountType,
            addonsCost: addonsData.cost,
            quantity: productProvider.quantity!,
          );

          // Check product availability
          final bool isAvailable = DateConverterHelper.isAvailable(
            widget.product!.availableTimeStarts!,
            widget.product!.availableTimeEnds!,
          );

          // Build cart model
          final CartModel cartModel = ProductHelper.buildCartModel(
              ...
              child: Container(
                ...
                child: Consumer<ProductProvider>(
                  builder: (context, productProvider, child) {
                    ...
                    cartProductWidget(
                      context, priceDetails.basePrice,
                      priceDetails.priceWithDiscount, variationList ?? [],
                      productProvider, priceDetails.totalWithDiscount,
      ...
      Consumer<ProductProvider>(
        builder: (context, productProvider, _) {
          ...
          return CustomButtonWidget(
            btnTxt: getTranslated((widget.cart != null || productProvider.cartIndex != -1) ? 'update_in_cart' : 'add_to_cart', context),
                ...
                Row(children: [
                  ...
                  if(widget.product?.reviewCount != null && (widget.product!.reviewCount! > 0))
                    InkWell(
                      ...
                      child: Row(children: [
                        Text("( ", style: rubikRegular.copyWith(color: Theme.of(context).primaryColor)),
