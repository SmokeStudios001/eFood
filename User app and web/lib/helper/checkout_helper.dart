...
class CheckOutHelper{
  ...
  static double getReferralDiscount({ReferralCustomerDetails? referralDetails, required double totalAmount}) {
    final bool isReferralAmountNotAvailable = (referralDetails?.isUsed ?? true);

    if (isReferralAmountNotAvailable) {
      return 0;
    }
  ...
  static void selectPaymentMethodAutomatically({required List<PaymentMethod> activePaymentMethodList, required bool isCashOnDeliveryActive}) {
    final CheckoutProvider checkoutProvider = Provider.of<CheckoutProvider>(Get.context!, listen: false);
