...
class ProductProvider extends DataSyncProvider {
  ...
  void initData(Product? product, CartModel? cart) {
    ...
    if(cart != null) {
      ...
      // Deep copy variations to avoid reference issues
      for (List<bool?> variation in cart.variations ?? []) {
        _selectedVariations.add(List.from(variation));
      }
      ...
      _quantity = 1;
      _cartIndex = -1; // Reset cart index when opening fresh (not editing from cart)
  ...
  void updateProductReviewShowStatus({bool isUpdate = true, bool value = false}){
  ...
  /// Checks if selected variations match an existing cart item and loads its state
  /// 
  /// This method updates the provider state based on whether a matching cart item exists:
  /// - If match found: loads quantity and addon state from the existing cart item
  /// - If no match: resets to default state (quantity=1, all addons inactive)
  /// 
  /// The matched cart index is stored in the `_cartIndex` field and can be accessed via the `cartIndex` getter.
  void setExistInCartForBottomSheet(Product product, List<List<bool?>> selectedVariations) {
    final cartProvider = Provider.of<CartProvider>(Get.context!, listen: false);
    
    final int matchedCartIndex = cartProvider.isExistInCartWithVariation(
      product.id, 
      null, 
      selectedVariations,
    );
    
    _cartIndex = matchedCartIndex;
    
    if (matchedCartIndex != -1) {
      final CartModel matchedCart = cartProvider.cartList[matchedCartIndex]!;
      
      _quantity = matchedCart.quantity;
      _addOnActiveList = ProductHelper.initializeCartAddonActiveList(
        product, 
        matchedCart.addOnIds!,
      );
      _addOnQtyList = ProductHelper.initializeCartAddonQuantityList(
        product, 
        matchedCart.addOnIds!,
      );
    } else {
