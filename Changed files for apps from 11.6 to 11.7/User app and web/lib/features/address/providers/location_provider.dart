...
import 'package:flutter_restaurant/helper/address_helper.dart';
...
class LocationProvider with ChangeNotifier {
  ...
  set setCameraPosition(CameraPosition? value)=> _cameraPosition = value;
  ...
  void onClearAddressList()=> _addressList?.clear();
  ...
  void setPickData({Position? getPosition, String? getAddress}) {
    _pickPosition = getPosition ?? _position;
    _pickAddress = getAddress ?? _address;
  }
  ...
  Future<String> getAddressFromGeocode(LatLng latLng) async {
    ...
    if(response.response?.statusCode == 200 && response.response!.data['status'] == 'OK') {
      address =  AddressHelper.buildDetailedAddress(response.response?.data['results']);
    }else {
