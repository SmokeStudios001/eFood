...
class GuestCheckoutWidget extends StatelessWidget {
  const GuestCheckoutWidget({super.key, required this.onGuestTap, this.checkoutRoute});
        ...
        SizedBox(height: 40, child: Row(children: [
          Expanded(
            child: CustomButtonWidget(
              ...
              onTap: () {
                context.pop();
                // Use checkout route if provided, otherwise use current route
                final redirectRoute = checkoutRoute ?? GoRouter.of(context).routeInformationProvider.value.uri.toString();
                return RouterHelper.getCreateAccountRoute(redirectRoute: redirectRoute);
              },
            ),
          ),
          ...
          Expanded(
            child: CustomButtonWidget(
              ...
              onTap: () {
                ...
                // Use checkout route if provided, otherwise use current route
                final redirectRoute = checkoutRoute ?? GoRouter.of(context).routeInformationProvider.value.uri.toString();
                RouterHelper.getLoginRoute(redirectRoute: redirectRoute);
              }
            ),
          ),
        ])),
