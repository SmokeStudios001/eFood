...
class _ExistingAccountBottomSheetState extends State<ExistingAccountBottomSheet> {
        ...
        Expanded(flex: 3, child: Consumer<AuthProvider>(
          builder: (context, authProvider, child) {
            return CustomButtonWidget(
              ...
              onTap: () => _handleNewAccountCreation(authProvider),
            );
          }
        ),),
        ...
        Expanded(flex: 3,child: Consumer<AuthProvider>(
          builder: (context, authProvider, child) {
            return CustomButtonWidget(
              ...
              onTap: () => _handleExistingAccountLogin(authProvider),
            );
          }
        ),),
  ...
  /// Handles the case where user indicates this is NOT their account
  /// Creates a new account with the provided information
  Future<void> _handleNewAccountCreation(AuthProvider authProvider) async {
    if (authProvider.isPhoneNumberVerificationButtonLoading) return;

    Navigator.pop(context);

    final (responseModel, tempToken) = await authProvider.existingAccountCheck(
    ...
    if (responseModel != null && responseModel.isSuccess && responseModel.message == 'tempToken') {
      RouterHelper.getOtpRegistrationScreen(
        tempToken!,
        widget.loginMedium == FromPage.otp.name 
            ? widget.userInfoModel.phone! 
  ...
  /// Handles the case where user confirms this IS their account
  /// Logs them in and redirects appropriately
  Future<void> _handleExistingAccountLogin(AuthProvider authProvider) async {
    if (authProvider.isPhoneNumberVerificationButtonLoading) return;
    ...
    final String? countryCode = NumberCheckerHelper.getCountryCode(widget.userInfoModel.phone);
    ...
    if (widget.redirectRoute?.isNotEmpty ?? false) {
