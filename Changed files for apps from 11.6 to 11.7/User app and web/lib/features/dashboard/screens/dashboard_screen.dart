...
class _DashboardScreenState extends State<DashboardScreen> with TickerProviderStateMixin {
  ...
  void didUpdateWidget(DashboardScreen oldWidget) {
    ...
    // Handle page index change after login redirect
    if (oldWidget.pageIndex != widget.pageIndex) {

      // Update the page controller and state
  ...
  void _showReferUseBottomSheet() {
    ...
    if(authProvider.isLoggedIn() && (config?.referEarnStatus ?? false) && (config?.customerReferredDiscountStatus ?? false)){
      profileProvider.getUserInfo(true, isUpdate: true).then((value) async {
        ...
        if( userData !=null && discountAmount > 0 && userData.referralCustomerDetails != null && userData.referralCustomerDetails!.isChecked == 0 && !(userData.referralCustomerDetails?.isUsed ?? true)){
  ...
  void _setPage(int pageIndex) {
    // Map page index to page name
    String pageName = _getPageNameFromIndex(pageIndex);
    
    // Check if page requires authentication
    final bool requiresAuth = pageName == 'order' || pageName == 'favourite';
    
    if (requiresAuth) {
      final authProvider = Provider.of<AuthProvider>(context, listen: false);
      if (!authProvider.isLoggedIn()) {
        // Not logged in - only update URL to trigger auth dialog via router
        RouterHelper.getDashboardRoute(pageName, action: RouteAction.pushReplacement);
        return;
      }
    }
    
    // User is authenticated (or page doesn't require auth)
    // Update the page controller and state to change the displayed page
    _pageController?.jumpToPage(pageIndex);
    setState(() {
      _pageIndex = pageIndex;
    });
    
    // Also update the URL for proper redirects after login
    RouterHelper.getDashboardRoute(pageName, action: RouteAction.pushReplacement);
  }

  String _getPageNameFromIndex(int index) {
    switch (index) {
      case 0:
        return 'home';
      case 1:
