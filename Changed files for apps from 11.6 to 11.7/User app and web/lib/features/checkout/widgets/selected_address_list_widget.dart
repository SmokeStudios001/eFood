...
class _SelectedAddressListWidgetState extends State<SelectedAddressListWidget> {
  ...
  /// Returns the page name based on the widget type
  String get _pageName => widget.isFromAppbar ? 'appbar' : 'checkout';
  
  /// Returns the route type based on the widget type
  RouteTypeEnum get _routeType => widget.isFromAppbar ? RouteTypeEnum.appbar : RouteTypeEnum.checkout;
  ...
  void initState() {
    ...
    if(locationProvider.addressList == null && isLoggedIn) {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<LocationProvider>(builder: (context, locationProvider, _) {
      final int addressListLength = locationProvider.addressList?.length ?? 0;
      final int itemCount = addressListLength + (widget.isFromAppbar ? 1 : 0);
      ...
      return CustomDialogShapeWidget(
        ...
        /// Handle different cases for address display
        /// Case 1: Not logged in - show empty state directly (no shimmer needed)
        /// Case 2: Logged in and addressList is null - show shimmer (loading)
        /// Case 3: Logged in and addressList is empty - show empty state
        /// Case 4: Logged in and addressList has items - show the list
        (isLoggedIn && locationProvider.addressList == null) ? const _AddressShimmerWidget(enabled: true,) :
        ...
        (locationProvider.addressList?.isNotEmpty ?? false) ? Expanded(child: Column(children: [
          ...
          Expanded(child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              // Show "Set from map" button as the last item when in appbar mode
              if (index == addressListLength && widget.isFromAppbar) {
                return const _SetFromMapButtonWidget();
              }
          ...
          if((locationProvider.addressList?.isNotEmpty ?? false) && isLoggedIn)
            TextButton.icon(
              onPressed: () async {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      child: Column( mainAxisSize: MainAxisSize.min, children: [
            ...
            return CustomButtonWidget(
              ...
              onTap: () async {
                ...
                final isSuccess = await locationProvider.checkPermission(
                      () async {
                    // Get current location and store in SelectLocationProvider
                    final currentLocation = await locationProvider.getCurrentLocation(context, true);
                    // selectLocationProvider.setSelectedAddress(currentLocation);

                    // Use the address from SelectLocationProvider
        ...
        if(isFromAppbar) ...[
          _SetFromMapButtonWidget(isPositionCenter: true),
...
class _SetFromMapButtonWidget extends StatelessWidget {
  final bool isPositionCenter;
  const _SetFromMapButtonWidget({this.isPositionCenter = false});

  @override
  Widget build(BuildContext context) {
    return Row(
      mainAxisAlignment: isPositionCenter ? MainAxisAlignment.center : MainAxisAlignment.start,
      children: [
        TextButton.icon(
          onPressed: () async {
            if (context.mounted) {
              ...
              Navigator.of(context).push(MaterialPageRoute(
                builder: (context) => SelectLocationScreen(
                  selectionMode: LocationSelectionMode.setCurrentLocation,
                  initialLocation: locationProvider.currentAddress,
                ),
              ));
            }
          },
          ...
          label: Text(
            getTranslated('set_from_map', context)!,
